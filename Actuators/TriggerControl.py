...
class TriggerControl:
    def __init__(self, mode=None):
        ...
        self.pwm_dart = pwmio.PWMOut(SERVO_DART_PIN, frequency=50)

        self.servo_x = servo.Servo(self.pwm_x)
        self.servo_z = servo.Servo(self.pwm_z)
        self.servo_dart = servo.Servo(self.pwm_dart)
        ...
        print(f"[Trigger] Initialized in {mode.upper()} mode.")
    ...
    def shoot(self):
        with self._lock:
            ...
            self.servo_dart.angle = DART_FIRE_ANGLE
            time.sleep(0.5)
            self.servo_dart.angle = DART_RETRACT_ANGLE
            time.sleep(0.5)
